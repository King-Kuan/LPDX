...
export default function Home() {
  ...
  const handleExport = useCallback(async (format: 'lpdx' | 'pdf' | 'docx') => {
    ...
    try {
      ...
      triggerDownload(bytes, `${safeTitle}.${ext}`);
      setExportDone(true);
        ...
        <div style={{ textAlign: 'center' }}>
          <div style={{ width: 48, height: 48, background: 'var(--accent)', borderRadius: 12, margin: '0 auto 16px', display: 'flex', alignItems: 'center', justifyContent: 'center', animation: 'pulse 1.5s ease-in-out infinite' }}>
            <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
              <rect x="2" y="1" width="12" height="16" rx="2" fill="white" opacity="0.9"/>
              <rect x="8" y="5" width="12" height="16" rx="2" fill="white" opacity="0.4"/>
            </svg>
          </div>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: 18, color: 'var(--text-primary)' }}>
            Loading LightDoc Studio…
          </div>
        </div>
      </div>
    );
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
      ...
      {showScan && (
        <ScanModal
          ...
          imageQuality={activeDoc.imageQuality}
        />
      )}
      {showImport && (
        <ImportModal
